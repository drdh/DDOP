...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world):
        ...
        for agent in world.agents:
            ...
            agent.state.p_pos[0] = np.random.uniform(-0.9, -0.7)
            agent.state.p_vel = np.zeros(world.dim_p)
            agent.state.c = np.zeros(world.dim_c)

        # Set random initial states
        world.landmarks[0].state.p_pos = np.array([-0.25, 0.85])
        world.landmarks[0].state.p_vel = np.zeros(world.dim_p)

        world.landmarks[1].state.p_pos = np.array([0.25, -0.85])
        ...
        # First seam
        world.walls[4].state.start = np.array([-0.5, -0.99])
        world.walls[4].state.end = np.array([-0.5, -0.06])

        world.walls[5].state.start = np.array([-0.5, 0.06])
        world.walls[5].state.end = np.array([-0.5, 0.99])

        # Second seam
        world.walls[6].state.start = np.array([0, -0.99])
        world.walls[6].state.end = np.array([0, -0.81])

        world.walls[7].state.start = np.array([0, -0.69])
        world.walls[7].state.end = np.array([0, 0.99])

        # Third seam
        world.walls[8].state.start = np.array([0.5, -0.99])
        world.walls[8].state.end = np.array([0.5, 0.69])

        world.walls[9].state.start = np.array([0.5, 0.81])
        world.walls[9].state.end = np.array([0.5, 0.99])
    ...
    def reward(self, agent, world):
        ...
        for i, l in enumerate(world.landmarks):
            dists = [np.sqrt(np.sum(np.square(a.state.p_pos - l.state.p_pos))) for a in world.agents]
            rew += (2 * i + 1) * max(0, 2*world.agents[0].size - min(dists))
        # if agent.collide:
        #     for a in world.agents:
        #         if self.is_collision(a, agent):
        #             rew -= 1
    ...
    def done(self, agent, world):
        ...
        for l in world.landmarks:
            done.append(min([(np.sqrt(np.sum(np.square(a.state.p_pos - l.state.p_pos))))
                            for a in world.agents]) < 2*world.agents[0].size)

        return np.array(done).all()
