...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world):
        ...
        for agent in world.agents:
            ...
            while not ((-0.2 < agent.state.p_pos[0] < 0.2) and (-0.2 < agent.state.p_pos[1] < 0.2)):
                agent.state.p_pos = np.random.uniform(-1, +1, world.dim_p)
        ...
        for i, landmark in enumerate(world.landmarks):
            ...
            while (-0.8 < landmark.state.p_pos[0] < 0.8) and (-0.8 < landmark.state.p_pos[1] < 0.8):
                landmark.state.p_pos = np.random.uniform(-0.85, 0.85, world.dim_p)
    ...
    def reward(self, agent, world):
        ...
        dists = []
        for l in world.landmarks:
            dists.append(min([np.sqrt(np.sum(np.square(a.state.p_pos - l.state.p_pos))) for a in world.agents]) <
                         2 * agent.size)

        if np.array(dists).all():
            rew += 100.0
