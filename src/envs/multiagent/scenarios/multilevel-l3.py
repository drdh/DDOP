...
class Scenario(BaseScenario):
    def make_world(self, neighbor_obs=None):
        ...
        for i, landmark in enumerate(world.landmarks):
            ...
            if i == 0:
                landmark.color = [0, 0, 1]
            else:
                landmark.color = [0, 1, 0]
    ...
    def reset_world(self, world):
        ...
        for i, landmark in enumerate(world.landmarks):
            landmark.color = np.array(landmark.color)
        ...
        for agent in world.agents:
            ...
            agent.state.p_pos[0] = np.random.uniform(-0.2, 0.2)
            agent.state.p_pos[1] = np.random.uniform(-0.9, -0.8)

            agent.state.p_vel = np.zeros(world.dim_p)
            agent.state.c = np.zeros(world.dim_c)

        # Set random initial states
        world.landmarks[0].state.p_pos = np.array([0.75, -0.85])
        world.landmarks[0].state.p_vel = np.zeros(world.dim_p)

        world.landmarks[1].state.p_pos = np.array([-0.75, 0.85])
        ...
        world.walls[6].state.start = np.array([0.0, -0.69])
        world.walls[6].state.end = np.array([0.0, 0.99])

        # Third seam
        world.walls[7].state.start = np.array([0.5, -0.99])
        world.walls[7].state.end = np.array([0.5, 0.69])

        world.walls[8].state.start = np.array([0.5, 0.81])
        world.walls[8].state.end = np.array([0.5, 0.99])
    ...
    def reward(self, agent, world):
        ...
        for i, landmark in enumerate(world.landmarks):
            if np.sqrt(np.sum(np.square(world.agents[i].state.p_pos - landmark.state.p_pos))) < 2 * agent.size:
                count += 1

        if count == 2 and agent.color in [[0,0,1], [0,1,0]]:
            rew += 100
