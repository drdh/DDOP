...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world):
        ...
        for i, landmark in enumerate(world.landmarks):
            ...
            landmark.state.p_pos[0] = np.random.uniform(0.7, 0.9)
        ...
        world.walls[4].state.start = np.array([0, -0.93])
        world.walls[4].state.end = np.array([0, -0.07])

        world.walls[5].state.start = np.array([0, 0.07])
        world.walls[5].state.end = np.array([0, 0.93])
    ...
    def reward(self, agent, world):
        ...
        for l in world.landmarks:
            dists = [np.sqrt(np.sum(np.square(a.state.p_pos - l.state.p_pos))) for a in world.agents]
            rew += max(0, 2*world.agents[0].size - min(dists))
        # if agent.collide:
        #     for a in world.agents:
        #         if self.is_collision(a, agent):
        #             rew -= 1
